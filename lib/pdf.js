...
import { buildQuickLinks, normalizeItinerary, parseItineraryJSON, linkifySegments, starStates, STAR_PATH, STAR_PATH_LEFT_HALF } from "./itinerary.js";

const C = { gold: "#B8962E", ink: "#1C1A17", dusk: "#4A3F35", stone: "#C8BFB0", mist: "#EAE4DA", sand: "#F2EDE4" };

// Drawn as vector shapes rather than a ★ text glyph — the PDF's base font
// (Helvetica) only covers the WinAnsi character set and has no star glyph
// at all, so this is the only reliable way to show a rating in the PDF.
// Half stars are a literal half-shaped path painted over a full one, rather
// than an SVG clipPath — react-pdf accepts a clipPath prop without error
// but doesn't actually apply it (verified by inspecting a generated PDF's
// content stream: zero clip regions were emitted for it).
function StarRatingPDF({ value }) {
  const states = starStates(value);
  if (!states.length) return null;
  return (
    <View style={{ flexDirection: "row", marginLeft: 4 }}>
      {states.map((state, i) => (
        <Svg key={i} width={8} height={8} viewBox="0 0 24 24" style={{ marginRight: 1 }}>
...
const s = StyleSheet.create({
  ...
  tocBox:     { borderWidth: 0.5, borderColor: C.stone, borderStyle: "solid", padding: 10, marginBottom: 16 },
  tocItem:    { fontSize: 8.5, color: C.gold, fontFamily: "Helvetica-Bold", lineHeight: 1.9, textDecoration: "none" },
...
function LegRow({ leg, internal }) {
  ...
  return (
    <View style={s.legRow}>
      <Text style={s.legLabel}>{internal ? "Internal flight" : leg.label}</Text>
  ...
  return (
    <View style={s.optRow}>
      <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
        ...
        <StarRatingPDF value={opt.rating} />
      </View>
  ...
  return (
    <Document>
      <Page size="A4" style={s.page} wrap>
        ...
        {/* Standard PDF viewers don't support the web preview's collapsible
            accordion sections, so this gives the closest practical
            equivalent: click a region to jump straight to it. */}
        {d.regions?.length > 1 && (
          <View style={s.tocBox}>
            <Text style={s.secLabel}>Quick Jump</Text>
            {d.regions.map((region, ri) => (
              <Link key={ri} src={`#region-${ri}`} style={s.tocItem}>{region.name}</Link>
            ))}
          </View>
        )}
        ...
        {d.regions?.map((region, ri) => (
          <View key={ri}>
            {/* Only the heading is kept unsplittable — the region as a whole
                (accommodation + every day) can easily run taller than one
                page, and wrap={false} on something that tall breaks
                react-pdf's layout engine, producing overlapping/duplicated
                text instead of a clean page break. */}
            <View wrap={false} id={`region-${ri}`}>
            ...
            {region.accommodation?.options?.length > 0 && (
              <View style={{ marginBottom: 8 }}>
                ...
                <Text style={{ ...s.accomNote, fontSize: 6.5, marginBottom: 0, marginTop: 3 }}>
                ...
                {region.accommodation.options.some((o) => Number(o.rating) > 0) && (
                  <Text style={{ ...s.accomNote, fontSize: 6.5, marginBottom: 0, marginTop: 1 }}>
              ...
              <View key={i}>
                ...
                {g.items.map((item, j) => (
                  <Text key={j} style={s.linkItem}>
                    {item.label}{item.note ? `: ${item.note}` : ""}{item.link ? ` (${item.link})` : ""}
                  </Text>
                ))}
              </View>
            ))}
