...
// The PDF's base font (Helvetica) only covers the WinAnsi character set, so
// ★/☆ (outside it) render as blank/missing glyphs — swap them for something
// Helvetica actually has. Free-text fields like accommodation ratings are
// where these show up (e.g. "4.5★"), typed by Jen in the admin editor.
  ...
  return (
    <View style={s.optRow}>
      <Text style={s.optLabel}>
        {opt.label}{opt.name ? `: ${opt.name}` : ""}{opt.rating ? ` (${pdfSafeText(opt.rating)})` : ""}
      </Text>
