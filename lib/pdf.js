...
import { getRouteMapBuffer } from "./map.js";
...
const s = StyleSheet.create({
  ...
  mapImage:   { width: "100%", marginBottom: 16, borderWidth: 0.5, borderColor: C.stone },
...
function ItineraryPDF({ itinerary, firstName, mapImage }) {
  ...
  return (
    <Document>
      <Page size="A4" style={s.page} wrap>
        ...
        {mapImage ? <Image src={{ data: mapImage, format: "png" }} style={s.mapImage} /> : null}
...
export async function generatePdf(itinerary, firstName, destinationName) {
  const raw = typeof itinerary === "string" ? parseItineraryJSON(itinerary) : itinerary;
  const d = normalizeItinerary(raw);
  const regionNames = (d?.regions || []).map((r) => r.name).filter(Boolean);

  // Best-effort — a failed/slow geocode or tile fetch shouldn't block the
  // PDF from generating, it just comes back without the map.
  let mapImage = null;
  if (regionNames.length) {
    try { mapImage = await getRouteMapBuffer(regionNames, destinationName); } catch {}
  }

  return await renderToBuffer(<ItineraryPDF itinerary={itinerary} firstName={firstName} mapImage={mapImage} />);
}
