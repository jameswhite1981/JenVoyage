...
// Turns [label](https://...) markdown-style links in free text into real
// clickable react-pdf <Link> nodes, for use as <Text> children.
function linkedNodes(text) {
  return linkifySegments(text).map((seg, i) =>
    seg.type === "link"
      ? <Link key={i} src={seg.url}>{seg.label}</Link>
      : seg.value
  );
}
