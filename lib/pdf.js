...
import { buildQuickLinks, normalizeItinerary, parseItineraryJSON } from "./itinerary.js";
  ...
  return (
    <Document>
      <Page size="A4" style={s.page} wrap>
        ...
        {d.intro ? <Text style={s.overview}>{d.intro}</Text> : null}

        {d.preTripNotes?.length > 0 && (
          <View style={s.noteBox}>
            {d.preTripNotes.map((n, i) => <Text key={i} style={s.noteText}>{n}</Text>)}
          </View>
        )}
        ...
        {d.regions?.map((region, ri) => (
          <View key={ri} wrap={false}>
            ...
            {region.whyHere ? <Text style={s.whyHere}>{region.whyHere}</Text> : null}
            ...
            {region.accommodation?.options?.length > 0 && (
              <View style={{ marginBottom: 8 }}>
                ...
                {region.accommodation.note ? <Text style={s.accomNote}>{region.accommodation.note}</Text> : null}
                {region.accommodation.options.map((opt, i) => <OptionRow key={i} opt={opt} recommended />)}
              </View>
            )}

            {region.gettingThereNote ? <Text style={s.gettingThere}>{region.gettingThereNote}</Text> : null}

            {region.days?.map((day) => (
              <View key={day.day} style={s.dayWrap}>
                <Text style={s.dayNum}>Day {day.day}{day.dateLabel ? ` — ${day.dateLabel}` : ""}{day.bookInAdvance ? "  ⚠ Book in advance" : ""}</Text>
                <Text style={s.dayTitle}>{day.title}</Text>
                {day.description ? <Text style={s.body}>{day.description}</Text> : null}
        ...
        {d.alternativeOperators?.length > 0 && (
          <View style={{ marginTop: 16 }}>
            ...
            {d.alternativeOperators.map((op, i) => <Text key={i} style={s.tip}>— {op}</Text>)}
          </View>
        )}
        ...
        {d.goodToKnow?.length > 0 && (
          <View style={{ marginTop: 12 }}>
            ...
            {d.goodToKnow.map((tip, i) => <Text key={i} style={s.tip}>— {tip}</Text>)}
          </View>
        )}
