...
// Separate cookies per role so an admin session and a customer session can
// coexist in the same browser — e.g. Jen testing a shareable link she just
// generated must not log her out of /admin.
...
export async function getSession(role = "user") {
  const jar = await cookies();
  const token = jar.get(COOKIES[role])?.value;
...
export async function clearSession(role = "user") {
  const jar = await cookies();
