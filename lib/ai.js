...
- Specific regions/states requested: ${brief.specificRegions || "none, suggest a sensible route"}
...
RULES:
1. Split the trip into 2-4 geographic regions/bases. Group activities GEOGRAPHICALLY to minimise travel. If specific regions/states were requested above, build the trip around those; only substitute or add others if the request is impractical (e.g. too far apart for the trip length).
...
9. The "costSummary" must contain EXACTLY these six line items, in this order, and no others: "Total flights" (sum of all flight legs), "Total accommodation" (sum of the RECOMMENDED accommodation option in each region — not the other options), "Total suggested activities" (sum of the day-by-day options/excursion costs), "Total transfers" (ferries/private transfers not already counted as flights — use "£0" if none), "Overall total" (sum of the previous four), "Total per person" (overall total divided by adults + children). Every value should be a total for the whole party, not per night.
...
Respond ONLY with this JSON, no markdown, no preamble:
{
  ...
  "costSummary": [
    ...
    {"label":"Total suggested activities","value":"£X,XXX"},
