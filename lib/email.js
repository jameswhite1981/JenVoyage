...
// Only needed for admin-authored free text (personalMessage below) dropped
// into an HTML email body — everywhere else interpolates fixed form fields.
...
export async function sendItineraryReady(email, firstName, destinationName, token, personalMessage, pdfBuffer) {
  const link = `${BASE}/api/auth/verify?token=${token}`;
  const messageBlock = personalMessage ? `
        <div style="background:#F2EDE4;border-left:3px solid #B8962E;padding:1rem 1.25rem;margin:1.5rem 0;font-size:0.95rem;line-height:1.8;color:#4A3F35;white-space:pre-wrap">${escapeHtml(personalMessage)}</div>
  ` : "";
  await getResend().emails.send({
    from: FROM,
    to: email,
    subject: `Your ${destinationName} itinerary is ready - Jen Voyage`,
    attachments: pdfBuffer ? [{ filename: `${destinationName} Itinerary.pdf`, content: pdfBuffer }] : undefined,
    html: `
      <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#1C1A17">
        <p style="font-size:0.75rem;letter-spacing:0.3em;text-transform:uppercase;color:#B8962E">Jen Voyage</p>
        <h1 style="font-size:1.8rem;font-weight:300">Your itinerary is ready, ${firstName}.</h1>
        <p style="font-size:1rem;line-height:1.8;color:#4A3F35">
          Your bespoke ${destinationName} itinerary is ready${pdfBuffer ? " — your PDF copy is attached to this email" : ""}. Click below to see your full day-by-day plan online.
        </p>${messageBlock}
        <a href="${link}" style="display:inline-block;background:#1C1A17;color:#FDFBF8;font-family:system-ui,sans-serif;font-size:0.8rem;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;padding:0.9rem 2rem;text-decoration:none;margin:1rem 0">
          View my itinerary →
        </a>
