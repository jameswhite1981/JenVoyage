...
// Shared by "create new template" (no initial data) and "edit existing
// template" (initial pre-populated from a fetched template) — both just
// save via the same id-or-not PATCH/POST logic in persistTemplate.
export default function TemplateEditorPage({ initial }) {
  const router = useRouter();
  const [name, setName] = useState(initial?.name || "");
  const [destinationName, setDestinationName] = useState(initial?.destinationName || "");
  const [draft, setDraft] = useState(initial?.draft || emptyItinerary());
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"2.5rem 1.5rem 5rem" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"1rem", paddingBottom:"1.5rem", borderBottom:`1px solid ${C.stone}`, marginBottom:"2rem" }}>
          <div>
            ...
            <h1 style={{ fontSize:"1.6rem", fontWeight:300, margin:"0.4rem 0 0.2rem" }}>{initial ? "Edit template" : "Create new template"}</h1>
          </div>
