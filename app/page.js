...
export default function JenVoyagePage() {
  ...
  // Shows a bouncing "scroll for more" hint on form steps whose content runs
  // taller than the viewport, so people don't miss fields below the fold.
  // Hides itself again once the user actually scrolls.
  useEffect(() => {
    if (screen !== "form") { setShowScrollHint(false); return; }
    const check = () => {
